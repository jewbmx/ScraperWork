...
class sources:
    ...
    def getSources(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, quality='720p', timeout=30):
        ...
        if content == 'movie':
            ...
            seen = set()  # clean up massive strings that are passed to workers.Thread
            aliases[:] = [d for d in aliases if (t := tuple(sorted(d.items()))) not in seen and not seen.add(t)]
