...
class source:
    ...
    def sources(self, url, hostDict):
        try:
            ...
            items = client_utils.parseDOM(page, 'td', attrs={'class': 'linkdom w3-center'})
            for item in items:
                try:
                    hoster = client_utils.parseDOM(item, 'a', attrs={'rel': 'nofollow ugc'}, ret='title')[0]
                    valid, host = source_utils.is_host_valid(hoster, hostDict)
                    if valid:
                        link = client_utils.parseDOM(item, 'a', attrs={'rel': 'nofollow ugc'}, ret='href')[0]
